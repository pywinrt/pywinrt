...
#: The file a projection package keeps a namespace's table in, beside the
#: ``__init__.py`` that loads it. The runtime that reads it spells the name for
#: itself, as both ends of the format spell the magic and the record sizes.
TABLE_NAME = "_table.pywinrt"

#: The text form of the same table, which is what the repository and a source
#: distribution carry and what this module reads.
TABLE_TEXT_NAME = f"{TABLE_NAME}.txt"
