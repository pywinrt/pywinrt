...
namespace xlang::meta::reader
{
    ...
    struct CustomModSig
    {
        ...
        {
            XLANG_ASSERT(
                m_cmod == ElementType::CModReqd || m_cmod == ElementType::CModOpt);
        }
    ...
    inline std::vector<CustomModSig> parse_cmods(
        table_base const* table, byte_view& data)
    {
        std::vector<CustomModSig> result;
        auto cursor = data;

        for (auto element_type = uncompress_enum<ElementType>(cursor);
             element_type == ElementType::CModOpt
             || element_type == ElementType::CModReqd;
    ...
    struct TypeSig
    {
        ...
        TypeSig(table_base const* table, byte_view& data)
            : m_is_szarray(parse_szarray(table, data)),
              ...
              m_element_type(parse_element_type(data)), m_type(ParseType(table, data))
        {
        }
    ...
    struct ParamSig
    {
        ParamSig(table_base const* table, byte_view& data)
            : m_cmod(parse_cmods(table, data)), m_byref(is_by_ref(data)),
              m_type(table, data)
        {
        }
    ...
    struct MethodDefSig
    {
        MethodDefSig(table_base const* table, byte_view& data)
            : m_calling_convention(uncompress_enum<CallingConvention>(data)),
              m_generic_param_count(
                  enum_mask(m_calling_convention, CallingConvention::Generic)
                          == CallingConvention::Generic
                      ? uncompress_unsigned(data)
                      : 0),
              m_param_count(uncompress_unsigned(data)), m_ret_type(table, data)
        {
    ...
    struct FieldSig
    {
        FieldSig(table_base const* table, byte_view& data)
            : m_calling_convention(check_convention(data)),
              m_cmod(parse_cmods(table, data)), m_type(table, data)
        {
        }
    ...
    struct PropertySig
    {
      ...
      private:
        ...
        {
            auto conv = read<CallingConvention>(data);
            if (enum_mask(conv, CallingConvention::Property)
    ...
    inline GenericTypeInstSig::GenericTypeInstSig(
        table_base const* table, byte_view& data)
        : m_class_or_value(uncompress_enum<ElementType>(data)),
          m_type(table, uncompress_unsigned(data)),
          m_generic_arg_count(uncompress_unsigned(data))
    {
        if (!(m_class_or_value == ElementType::Class
              || m_class_or_value == ElementType::ValueType))
        {
            throw_invalid(
                "Generic type instantiation signatures must begin with either ELEMENT_TYPE_CLASS or ELEMENT_TYPE_VALUE");
        }
    ...
    inline TypeSig::value_type TypeSig::ParseType(
        table_base const* table, byte_view& data)
    {
