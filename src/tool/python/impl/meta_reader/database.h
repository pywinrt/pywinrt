...
namespace pywinrt::meta::reader
{
    ...
    struct database
    {
        ...
        static bool is_database(std::string_view const& path)
        {
            ...
            if (pe.FileHeader.NumberOfSections == 0
                || pe.FileHeader.NumberOfSections > 100)
            {
                return false;
            }
            ...
            {
                com_virtual_address
                    = pe.OptionalHeader.DataDirectory[14]
                          .VirtualAddress; // IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR
                sections = &file.as<impl::image_section_header>(
                    dos.e_lfanew + sizeof(impl::image_nt_headers32));
            }
            else if (pe.OptionalHeader.Magic == 0x20B) // PE32+
            {
                auto pe_plus = file.as<impl::image_nt_headers32plus>(dos.e_lfanew);
                com_virtual_address
                    = pe_plus.OptionalHeader.DataDirectory[14]
                          .VirtualAddress; // IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR
                sections = &file.as<impl::image_section_header>(
                    dos.e_lfanew + sizeof(impl::image_nt_headers32plus));
            }
            else
            {
            ...
            auto section
                = section_from_rva(sections, sections_end, com_virtual_address);
            ...
            section
                = section_from_rva(sections, sections_end, cli.MetaData.VirtualAddress);
        ...
        explicit database(std::vector<uint8_t>&& buffer, cache const* cache = nullptr)
            : m_buffer{std::move(buffer)},
              m_view{m_buffer.data(), m_buffer.data() + m_buffer.size()}, m_cache{cache}
        {
            initialize();
        }

        explicit database(std::string_view const& path, cache const* cache = nullptr)
            : m_view{path}, m_path{path}, m_cache{cache}
        {
            initialize();
        }
        ...
        std::string_view get_string(uint32_t const index) const
        {
            ...
            return {
                reinterpret_cast<char const*>(view.begin()),
                static_cast<uint32_t>(last - view.begin())};
        }
      ...
      private:
        void initialize()
        {
            ...
            if (pe.OptionalHeader.Magic == 0x10B) // PE32
            {
                com_virtual_address
                    = pe.OptionalHeader.DataDirectory[14]
                          .VirtualAddress; // IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR
                sections = &m_view.as<impl::image_section_header>(
                    dos.e_lfanew + sizeof(impl::image_nt_headers32));
            }
            else if (pe.OptionalHeader.Magic == 0x20B) // PE32+
            {
            ...
            for (uint32_t i{}; i < 64; ++i)
            {
                ...
                {
                case 0x00:
                    Module.set_row_count(row_count);
                    break;
                case 0x01:
                    TypeRef.set_row_count(row_count);
                    break;
                case 0x02:
                    TypeDef.set_row_count(row_count);
                    break;
                case 0x04:
                    Field.set_row_count(row_count);
                    break;
                case 0x06:
                    MethodDef.set_row_count(row_count);
                    break;
                case 0x08:
                    Param.set_row_count(row_count);
                    break;
                case 0x09:
                    InterfaceImpl.set_row_count(row_count);
                    break;
                case 0x0a:
                    MemberRef.set_row_count(row_count);
                    break;
                case 0x0b:
                    Constant.set_row_count(row_count);
                    break;
                case 0x0c:
                    CustomAttribute.set_row_count(row_count);
                    break;
                case 0x0d:
                    FieldMarshal.set_row_count(row_count);
                    break;
                case 0x0e:
                    DeclSecurity.set_row_count(row_count);
                    break;
                case 0x0f:
                    ClassLayout.set_row_count(row_count);
                    break;
                case 0x10:
                    FieldLayout.set_row_count(row_count);
                    break;
                case 0x11:
                    StandAloneSig.set_row_count(row_count);
                    break;
                case 0x12:
                    EventMap.set_row_count(row_count);
                    break;
                case 0x14:
                    Event.set_row_count(row_count);
                    break;
                case 0x15:
                    PropertyMap.set_row_count(row_count);
                    break;
                case 0x17:
                    Property.set_row_count(row_count);
                    break;
                case 0x18:
                    MethodSemantics.set_row_count(row_count);
                    break;
                case 0x19:
                    MethodImpl.set_row_count(row_count);
                    break;
                case 0x1a:
                    ModuleRef.set_row_count(row_count);
                    break;
                case 0x1b:
                    TypeSpec.set_row_count(row_count);
                    break;
                case 0x1c:
                    ImplMap.set_row_count(row_count);
                    break;
                case 0x1d:
                    FieldRVA.set_row_count(row_count);
                    break;
                case 0x20:
                    Assembly.set_row_count(row_count);
                    break;
                case 0x21:
                    AssemblyProcessor.set_row_count(row_count);
                    break;
                case 0x22:
                    AssemblyOS.set_row_count(row_count);
                    break;
                case 0x23:
                    AssemblyRef.set_row_count(row_count);
                    break;
                case 0x24:
                    AssemblyRefProcessor.set_row_count(row_count);
                    break;
                case 0x25:
                    AssemblyRefOS.set_row_count(row_count);
                    break;
                case 0x26:
                    File.set_row_count(row_count);
                    break;
                case 0x27:
                    ExportedType.set_row_count(row_count);
                    break;
                case 0x28:
                    ManifestResource.set_row_count(row_count);
                    break;
                case 0x29:
                    NestedClass.set_row_count(row_count);
                    break;
                case 0x2a:
                    GenericParam.set_row_count(row_count);
                    break;
                case 0x2b:
                    MethodSpec.set_row_count(row_count);
                    break;
                case 0x2c:
                    GenericParamConstraint.set_row_count(row_count);
                    break;
                default:
                    throw_invalid("Unknown metadata table");
                };
            }
            ...
            auto const HasCustomAttribute = composite_index_size(
                MethodDef,
                Field,
                TypeRef,
                TypeDef,
            ...
            auto const HasDeclSecurity
                = composite_index_size(TypeDef, MethodDef, Assembly);
            auto const MemberRefParent = composite_index_size(
            ...
            auto const Implementation
                = composite_index_size(File, AssemblyRef, ExportedType);
            auto const CustomAttributeType = composite_index_size(
                MethodDef, MemberRef, empty_table, empty_table, empty_table);
            auto const ResolutionScope
                = composite_index_size(Module, ModuleRef, AssemblyRef, TypeRef);
            auto const TypeOrMethodDef = composite_index_size(TypeDef, MethodDef);

            Assembly.set_columns(
                4, 8, 4, blob_index_size, string_index_size, string_index_size);
            AssemblyOS.set_columns(4, 4, 4);
            AssemblyProcessor.set_columns(4);
            AssemblyRef.set_columns(
                8,
                4,
                blob_index_size,
                string_index_size,
            ...
            CustomAttribute.set_columns(
                HasCustomAttribute, CustomAttributeType, blob_index_size);
            ...
            ExportedType.set_columns(
                4, 4, string_index_size, string_index_size, Implementation);
            ...
            ImplMap.set_columns(
                2, MemberForwarded, string_index_size, ModuleRef.index_size());
            ...
            MethodDef.set_columns(
                4, 2, 2, string_index_size, blob_index_size, Param.index_size());
            MethodImpl.set_columns(
                TypeDef.index_size(), MethodDefOrRef, MethodDefOrRef);
        ...
        {
            return (composite_index_size(
                        tables.size(), impl::bits_needed(sizeof...(tables)))
                    && ...)
                       ? 2
                       : 4;
        }
        ...
        static impl::image_section_header const* section_from_rva(
            impl::image_section_header const* const first,
            impl::image_section_header const* const last,
            uint32_t const rva) noexcept
        {
            return std::find_if(
                first,
                last,
                [rva](auto&& section) noexcept
                {
        ...
        static uint32_t offset_from_rva(
            impl::image_section_header const& section, uint32_t const rva) noexcept
        {
    ...
    template<>
    inline table<Module> const& database::get_table<Module>() const noexcept
    {
        return Module;
    }
    template<>
    inline table<TypeRef> const& database::get_table<TypeRef>() const noexcept
    {
        return TypeRef;
    }
    template<>
    inline table<TypeDef> const& database::get_table<TypeDef>() const noexcept
    {
        return TypeDef;
    }
    template<>
    inline table<Field> const& database::get_table<Field>() const noexcept
    {
        return Field;
    }
    template<>
    inline table<MethodDef> const& database::get_table<MethodDef>() const noexcept
    {
        return MethodDef;
    }
    template<>
    inline table<Param> const& database::get_table<Param>() const noexcept
    {
        return Param;
    }
    template<>
    inline table<InterfaceImpl> const& database::get_table<InterfaceImpl>()
        const noexcept
    {
        return InterfaceImpl;
    }
    template<>
    inline table<MemberRef> const& database::get_table<MemberRef>() const noexcept
    {
        return MemberRef;
    }
    template<>
    inline table<Constant> const& database::get_table<Constant>() const noexcept
    {
        return Constant;
    }
    template<>
    inline table<CustomAttribute> const& database::get_table<CustomAttribute>()
        const noexcept
    {
        return CustomAttribute;
    }
    template<>
    inline table<FieldMarshal> const& database::get_table<FieldMarshal>() const noexcept
    {
        return FieldMarshal;
    }
    template<>
    inline table<DeclSecurity> const& database::get_table<DeclSecurity>() const noexcept
    {
        return DeclSecurity;
    }
    template<>
    inline table<ClassLayout> const& database::get_table<ClassLayout>() const noexcept
    {
        return ClassLayout;
    }
    template<>
    inline table<FieldLayout> const& database::get_table<FieldLayout>() const noexcept
    {
        return FieldLayout;
    }
    template<>
    inline table<StandAloneSig> const& database::get_table<StandAloneSig>()
        const noexcept
    {
        return StandAloneSig;
    }
    template<>
    inline table<EventMap> const& database::get_table<EventMap>() const noexcept
    {
        return EventMap;
    }
    template<>
    inline table<Event> const& database::get_table<Event>() const noexcept
    {
        return Event;
    }
    template<>
    inline table<PropertyMap> const& database::get_table<PropertyMap>() const noexcept
    {
        return PropertyMap;
    }
    template<>
    inline table<Property> const& database::get_table<Property>() const noexcept
    {
        return Property;
    }
    template<>
    inline table<MethodSemantics> const& database::get_table<MethodSemantics>()
        const noexcept
    {
        return MethodSemantics;
    }
    template<>
    inline table<MethodImpl> const& database::get_table<MethodImpl>() const noexcept
    {
        return MethodImpl;
    }
    template<>
    inline table<ModuleRef> const& database::get_table<ModuleRef>() const noexcept
    {
        return ModuleRef;
    }
    template<>
    inline table<TypeSpec> const& database::get_table<TypeSpec>() const noexcept
    {
        return TypeSpec;
    }
    template<>
    inline table<ImplMap> const& database::get_table<ImplMap>() const noexcept
    {
        return ImplMap;
    }
    template<>
    inline table<FieldRVA> const& database::get_table<FieldRVA>() const noexcept
    {
        return FieldRVA;
    }
    template<>
    inline table<Assembly> const& database::get_table<Assembly>() const noexcept
    {
        return Assembly;
    }
    template<>
    inline table<AssemblyProcessor> const& database::get_table<AssemblyProcessor>()
        const noexcept
    {
        return AssemblyProcessor;
    }
    template<>
    inline table<AssemblyOS> const& database::get_table<AssemblyOS>() const noexcept
    {
        return AssemblyOS;
    }
    template<>
    inline table<AssemblyRef> const& database::get_table<AssemblyRef>() const noexcept
    {
        return AssemblyRef;
    }
    template<>
    inline table<AssemblyRefProcessor> const& database::get_table<
        AssemblyRefProcessor>() const noexcept
    {
        return AssemblyRefProcessor;
    }
    template<>
    inline table<AssemblyRefOS> const& database::get_table<AssemblyRefOS>()
        const noexcept
    {
        return AssemblyRefOS;
    }
    template<>
    inline table<File> const& database::get_table<File>() const noexcept
    {
        return File;
    }
    template<>
    inline table<ExportedType> const& database::get_table<ExportedType>() const noexcept
    {
        return ExportedType;
    }
    template<>
    inline table<ManifestResource> const& database::get_table<ManifestResource>()
        const noexcept
    {
        return ManifestResource;
    }
    template<>
    inline table<NestedClass> const& database::get_table<NestedClass>() const noexcept
    {
        return NestedClass;
    }
    template<>
    inline table<GenericParam> const& database::get_table<GenericParam>() const noexcept
    {
        return GenericParam;
    }
    template<>
    inline table<MethodSpec> const& database::get_table<MethodSpec>() const noexcept
    {
        return MethodSpec;
    }
    template<>
    inline table<GenericParamConstraint> const& database::get_table<
        GenericParamConstraint>() const noexcept
    {
