...
def skip_without_projection() -> None:
    ...
    their members, closes their parameterized interfaces, passes arrays and
    answers calls that come back the other way, so what is still missing is the
    buffer protocol, a Python list or dict passed where WinRT wants a
    collection, Python subclasses of composable classes, the numeric members of
    the ``Numerics`` structs and the compiled interop modules. Calling this
    before a module's imports keeps the suite reporting skips rather than a
    wall of errors. Delete the call from a module once the interpreter covers
    what it tests; ``test_projection.py``, ``test_delegates.py``,
    ``test_implements.py`` and ``test_array.py`` cover the part that is already
    there.
    """
