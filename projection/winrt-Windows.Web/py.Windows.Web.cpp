...
PyMODINIT_FUNC PyInit__winrt_windows_web(void) noexcept
{
    ...
    py::pyobj_handle WebError_type{py::register_python_type(module.get(), &type_spec_WebError, object_bases.get(), reinterpret_cast<PyTypeObject*>(type_WebError_Static.get()))};
    if (!WebError_type)
    {
        return nullptr;
    }

    py::pyobj_handle IUriToStreamResolver_type{py::register_python_type(module.get(), &type_spec_IUriToStreamResolver, object_bases.get(), nullptr)};
