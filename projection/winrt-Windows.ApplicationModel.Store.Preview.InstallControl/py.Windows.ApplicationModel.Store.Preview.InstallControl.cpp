...
PyMODINIT_FUNC PyInit__winrt_windows_applicationmodel_store_preview_installcontrol(void) noexcept
{
    ...
    py::pyobj_handle AppInstallItem_type{py::register_python_type(module.get(), &type_spec_AppInstallItem, object_bases.get(), nullptr)};
    if (!AppInstallItem_type)
    {
        return nullptr;
    }

    py::pyobj_handle AppInstallManager_type{py::register_python_type(module.get(), &type_spec_AppInstallManager, object_bases.get(), nullptr)};
    if (!AppInstallManager_type)
    {
        return nullptr;
    }

    py::pyobj_handle AppInstallManagerItemEventArgs_type{py::register_python_type(module.get(), &type_spec_AppInstallManagerItemEventArgs, object_bases.get(), nullptr)};
    if (!AppInstallManagerItemEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle AppInstallOptions_type{py::register_python_type(module.get(), &type_spec_AppInstallOptions, object_bases.get(), nullptr)};
    if (!AppInstallOptions_type)
    {
        return nullptr;
    }

    py::pyobj_handle AppInstallStatus_type{py::register_python_type(module.get(), &type_spec_AppInstallStatus, object_bases.get(), nullptr)};
    if (!AppInstallStatus_type)
    {
        return nullptr;
    }

    py::pyobj_handle AppUpdateOptions_type{py::register_python_type(module.get(), &type_spec_AppUpdateOptions, object_bases.get(), nullptr)};
    if (!AppUpdateOptions_type)
    {
        return nullptr;
    }

    py::pyobj_handle GetEntitlementResult_type{py::register_python_type(module.get(), &type_spec_GetEntitlementResult, object_bases.get(), nullptr)};
