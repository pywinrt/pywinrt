...
PyMODINIT_FUNC PyInit__winrt_Windows_Web_AtomPub(void) noexcept
{
    ...
    state->type_AtomPubClient = py::register_python_type(module.get(), type_name_AtomPubClient, &type_spec_AtomPubClient, object_bases.get(), nullptr);
    if (!state->type_AtomPubClient)
    {
        return nullptr;
    }

    state->type_ResourceCollection = py::register_python_type(module.get(), type_name_ResourceCollection, &type_spec_ResourceCollection, object_bases.get(), nullptr);
    if (!state->type_ResourceCollection)
    {
        return nullptr;
    }

    state->type_ServiceDocument = py::register_python_type(module.get(), type_name_ServiceDocument, &type_spec_ServiceDocument, object_bases.get(), nullptr);
    if (!state->type_ServiceDocument)
    {
        return nullptr;
    }

    state->type_Workspace = py::register_python_type(module.get(), type_name_Workspace, &type_spec_Workspace, object_bases.get(), nullptr);
