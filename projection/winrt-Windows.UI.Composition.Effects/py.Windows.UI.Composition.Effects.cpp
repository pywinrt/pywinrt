...
PyMODINIT_FUNC PyInit__winrt_Windows_UI_Composition_Effects(void) noexcept
{
    ...
    state->type_SceneLightingEffect = py::register_python_type(module.get(), type_name_SceneLightingEffect, &type_spec_SceneLightingEffect, object_bases.get(), nullptr);
    if (!state->type_SceneLightingEffect)
    {
        return nullptr;
    }
...
PyTypeObject* py::winrt_type<winrt::Windows::UI::Composition::Effects::SceneLightingEffect>::get_python_type() noexcept {
    ...
    auto python_type = state->type_SceneLightingEffect;

    if (!python_type) {
        PyErr_SetString(PyExc_RuntimeError, "type winrt::Windows::UI::Composition::Effects::SceneLightingEffect is not registered");
        return nullptr;
    }
