...
PyMODINIT_FUNC PyInit__winrt_windows_applicationmodel_datatransfer_sharetarget(void) noexcept
{
    ...
    if (py::register_python_type(module.get(), &type_spec_QuickLink, nullptr, object_bases.get(), nullptr) == -1)
    #else
    ...
    #if PY_VERSION_HEX < 0x03090000
    if (py::register_python_type(module.get(), &type_spec_ShareOperation, nullptr, object_bases.get(), nullptr) == -1)
    #else
