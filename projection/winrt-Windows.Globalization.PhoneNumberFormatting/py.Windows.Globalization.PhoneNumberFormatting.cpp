...
PyMODINIT_FUNC PyInit__winrt_Windows_Globalization_PhoneNumberFormatting(void) noexcept
{
    ...
    state->type_PhoneNumberFormatter = py::register_python_type(module.get(), type_name_PhoneNumberFormatter, &type_spec_PhoneNumberFormatter, object_bases.get(), nullptr);
    if (!state->type_PhoneNumberFormatter)
    {
        return nullptr;
    }

    state->type_PhoneNumberInfo = py::register_python_type(module.get(), type_name_PhoneNumberInfo, &type_spec_PhoneNumberInfo, object_bases.get(), nullptr);
