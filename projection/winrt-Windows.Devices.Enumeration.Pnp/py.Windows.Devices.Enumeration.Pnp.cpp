...
PyMODINIT_FUNC PyInit__winrt_Windows_Devices_Enumeration_Pnp(void) noexcept
{
    ...
    state->type_PnpObject = py::register_python_type(module.get(), type_name_PnpObject, &type_spec_PnpObject, object_bases.get(), nullptr);
    if (!state->type_PnpObject)
    {
        return nullptr;
    }

    state->type_PnpObjectCollection = py::register_python_type(module.get(), type_name_PnpObjectCollection, &type_spec_PnpObjectCollection, object_bases.get(), nullptr);
    if (!state->type_PnpObjectCollection)
    {
        return nullptr;
    }

    state->type_PnpObjectUpdate = py::register_python_type(module.get(), type_name_PnpObjectUpdate, &type_spec_PnpObjectUpdate, object_bases.get(), nullptr);
    if (!state->type_PnpObjectUpdate)
    {
        return nullptr;
    }

    state->type_PnpObjectWatcher = py::register_python_type(module.get(), type_name_PnpObjectWatcher, &type_spec_PnpObjectWatcher, object_bases.get(), nullptr);
