...
PyMODINIT_FUNC PyInit__winrt_windows_devices_enumeration_pnp(void) noexcept
{
    ...
    if (py::register_python_type(module.get(), &type_spec_PnpObject, nullptr, object_bases.get(), nullptr) == -1)
    #else
    ...
    #if PY_VERSION_HEX < 0x03090000
    if (py::register_python_type(module.get(), &type_spec_PnpObjectCollection, nullptr, object_bases.get(), nullptr) == -1)
    #else
    ...
    #if PY_VERSION_HEX < 0x03090000
    if (py::register_python_type(module.get(), &type_spec_PnpObjectUpdate, nullptr, object_bases.get(), nullptr) == -1)
    #else
    ...
    #if PY_VERSION_HEX < 0x03090000
    if (py::register_python_type(module.get(), &type_spec_PnpObjectWatcher, nullptr, object_bases.get(), nullptr) == -1)
    #else
