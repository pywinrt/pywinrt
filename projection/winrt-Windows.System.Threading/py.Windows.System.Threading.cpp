...
PyMODINIT_FUNC PyInit__winrt_Windows_System_Threading(void) noexcept
{
    ...
    if (py::register_python_type(module.get(), type_name_ThreadPool, &type_spec_ThreadPool, nullptr, object_bases.get(), nullptr) == -1)
    #else
    if (py::register_python_type(module.get(), type_name_ThreadPool, &type_spec_ThreadPool, object_bases.get(), nullptr) == -1)
    #endif
    {
        return nullptr;
    }
    ...
    #if PY_VERSION_HEX < 0x03090000
    if (py::register_python_type(module.get(), type_name_ThreadPoolTimer, &type_spec_ThreadPoolTimer, nullptr, object_bases.get(), nullptr) == -1)
    #else
    if (py::register_python_type(module.get(), type_name_ThreadPoolTimer, &type_spec_ThreadPoolTimer, object_bases.get(), nullptr) == -1)
    #endif
    {
        return nullptr;
    }
