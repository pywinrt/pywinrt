...
PyMODINIT_FUNC PyInit__winrt_Windows_ApplicationModel_Store_LicenseManagement(void) noexcept
{
    ...
    state->type_LicenseManager = py::register_python_type(module.get(), type_name_LicenseManager, &type_spec_LicenseManager, object_bases.get(), nullptr);
    if (!state->type_LicenseManager)
    {
        return nullptr;
    }

    state->type_LicenseSatisfactionInfo = py::register_python_type(module.get(), type_name_LicenseSatisfactionInfo, &type_spec_LicenseSatisfactionInfo, object_bases.get(), nullptr);
    if (!state->type_LicenseSatisfactionInfo)
    {
        return nullptr;
    }

    state->type_LicenseSatisfactionResult = py::register_python_type(module.get(), type_name_LicenseSatisfactionResult, &type_spec_LicenseSatisfactionResult, object_bases.get(), nullptr);
