...
PyMODINIT_FUNC PyInit__winrt_windows_applicationmodel_store_licensemanagement(void) noexcept
{
    ...
    py::pyobj_handle LicenseManager_type{py::register_python_type(module.get(), &type_spec_LicenseManager, object_bases.get(), reinterpret_cast<PyTypeObject*>(type_LicenseManager_Static.get()))};
    if (!LicenseManager_type)
    {
        return nullptr;
    }

    py::pyobj_handle LicenseSatisfactionInfo_type{py::register_python_type(module.get(), &type_spec_LicenseSatisfactionInfo, object_bases.get(), nullptr)};
    if (!LicenseSatisfactionInfo_type)
    {
        return nullptr;
    }

    py::pyobj_handle LicenseSatisfactionResult_type{py::register_python_type(module.get(), &type_spec_LicenseSatisfactionResult, object_bases.get(), nullptr)};
