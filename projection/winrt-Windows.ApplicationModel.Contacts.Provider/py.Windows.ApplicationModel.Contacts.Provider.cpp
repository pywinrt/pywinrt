...
namespace py::cpp::Windows::ApplicationModel::Contacts::Provider
{
    ...
    static constexpr const char* const type_name_ContactRemovedEventArgs = "ContactRemovedEventArgs";
...
PyMODINIT_FUNC PyInit__winrt_windows_applicationmodel_contacts_provider(void) noexcept
{
    ...
    if (py::register_python_type(module.get(), type_name_ContactPickerUI, &type_spec_ContactPickerUI, nullptr, object_bases.get(), nullptr) == -1)
    #else
    if (py::register_python_type(module.get(), type_name_ContactPickerUI, &type_spec_ContactPickerUI, object_bases.get(), nullptr) == -1)
    #endif
    {
        return nullptr;
    }
    ...
    #if PY_VERSION_HEX < 0x03090000
    if (py::register_python_type(module.get(), type_name_ContactRemovedEventArgs, &type_spec_ContactRemovedEventArgs, nullptr, object_bases.get(), nullptr) == -1)
    #else
    if (py::register_python_type(module.get(), type_name_ContactRemovedEventArgs, &type_spec_ContactRemovedEventArgs, object_bases.get(), nullptr) == -1)
    #endif
    {
        return nullptr;
    }
