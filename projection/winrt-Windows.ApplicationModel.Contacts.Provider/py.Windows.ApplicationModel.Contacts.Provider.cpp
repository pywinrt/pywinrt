...
PyMODINIT_FUNC PyInit__winrt_Windows_ApplicationModel_Contacts_Provider(void) noexcept
{
    ...
    state->type_ContactPickerUI = py::register_python_type(module.get(), type_name_ContactPickerUI, &type_spec_ContactPickerUI, object_bases.get(), nullptr);
    if (!state->type_ContactPickerUI)
    {
        return nullptr;
    }

    state->type_ContactRemovedEventArgs = py::register_python_type(module.get(), type_name_ContactRemovedEventArgs, &type_spec_ContactRemovedEventArgs, object_bases.get(), nullptr);
