...
PyMODINIT_FUNC PyInit__winrt_Windows_Phone_System_UserProfile_GameServices_Core(void) noexcept
{
    ...
    state->type_GameService = py::register_python_type(module.get(), type_name_GameService, &type_spec_GameService, object_bases.get(), reinterpret_cast<PyTypeObject*>(type_GameService_Meta.get()));
    if (!state->type_GameService)
    {
        return nullptr;
    }

    state->type_GameServicePropertyCollection = py::register_python_type(module.get(), type_name_GameServicePropertyCollection, &type_spec_GameServicePropertyCollection, object_bases.get(), nullptr);
