...
PyMODINIT_FUNC PyInit__winrt_Windows_Phone_System_UserProfile_GameServices_Core(void) noexcept
{
    ...
    if (py::register_python_type(module.get(), type_name_GameService, &type_spec_GameService, nullptr, object_bases.get(), reinterpret_cast<PyTypeObject*>(type_GameService_Meta.get())) == -1)
    #else
    if (py::register_python_type(module.get(), type_name_GameService, &type_spec_GameService, object_bases.get(), reinterpret_cast<PyTypeObject*>(type_GameService_Meta.get())) == -1)
    #endif
    {
        return nullptr;
    }
    ...
    #if PY_VERSION_HEX < 0x03090000
    if (py::register_python_type(module.get(), type_name_GameServicePropertyCollection, &type_spec_GameServicePropertyCollection, nullptr, object_bases.get(), nullptr) == -1)
    #else
    if (py::register_python_type(module.get(), type_name_GameServicePropertyCollection, &type_spec_GameServicePropertyCollection, object_bases.get(), nullptr) == -1)
    #endif
    {
        return nullptr;
    }
