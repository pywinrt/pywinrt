...
PyMODINIT_FUNC PyInit__winrt_Windows_System_Diagnostics_Telemetry(void) noexcept
{
    ...
    if (py::register_python_type(module.get(), type_name_PlatformTelemetryClient, &type_spec_PlatformTelemetryClient, nullptr, object_bases.get(), nullptr) == -1)
    #else
    if (py::register_python_type(module.get(), type_name_PlatformTelemetryClient, &type_spec_PlatformTelemetryClient, object_bases.get(), nullptr) == -1)
    #endif
    {
        return nullptr;
    }
    ...
    #if PY_VERSION_HEX < 0x03090000
    if (py::register_python_type(module.get(), type_name_PlatformTelemetryRegistrationResult, &type_spec_PlatformTelemetryRegistrationResult, nullptr, object_bases.get(), nullptr) == -1)
    #else
    if (py::register_python_type(module.get(), type_name_PlatformTelemetryRegistrationResult, &type_spec_PlatformTelemetryRegistrationResult, object_bases.get(), nullptr) == -1)
    #endif
    {
        return nullptr;
    }
    ...
    #if PY_VERSION_HEX < 0x03090000
    if (py::register_python_type(module.get(), type_name_PlatformTelemetryRegistrationSettings, &type_spec_PlatformTelemetryRegistrationSettings, nullptr, object_bases.get(), nullptr) == -1)
    #else
    if (py::register_python_type(module.get(), type_name_PlatformTelemetryRegistrationSettings, &type_spec_PlatformTelemetryRegistrationSettings, object_bases.get(), nullptr) == -1)
    #endif
    {
        return nullptr;
    }
