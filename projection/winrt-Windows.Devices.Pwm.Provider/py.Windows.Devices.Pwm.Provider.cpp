...
PyMODINIT_FUNC PyInit__winrt_windows_devices_pwm_provider(void) noexcept
{
    ...
    if (py::register_python_type(module.get(), type_name_IPwmControllerProvider, &type_spec_IPwmControllerProvider, nullptr, object_bases.get(), nullptr) == -1)
    #else
    if (py::register_python_type(module.get(), type_name_IPwmControllerProvider, &type_spec_IPwmControllerProvider, object_bases.get(), nullptr) == -1)
    #endif
    {
        return nullptr;
    }
    ...
    #if PY_VERSION_HEX < 0x03090000
    if (py::register_python_type(module.get(), type_name_IPwmProvider, &type_spec_IPwmProvider, nullptr, object_bases.get(), nullptr) == -1)
    #else
    if (py::register_python_type(module.get(), type_name_IPwmProvider, &type_spec_IPwmProvider, object_bases.get(), nullptr) == -1)
    #endif
    {
        return nullptr;
    }
