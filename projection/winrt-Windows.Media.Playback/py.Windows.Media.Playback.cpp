...
PyMODINIT_FUNC PyInit__winrt_windows_media_playback(void) noexcept
{
    ...
    py::pyobj_handle BackgroundMediaPlayer_type{py::register_python_type(module.get(), &type_spec_BackgroundMediaPlayer, object_bases.get(), reinterpret_cast<PyTypeObject*>(type_BackgroundMediaPlayer_Static.get()))};
    if (!BackgroundMediaPlayer_type)
    {
        return nullptr;
    }

    py::pyobj_handle CurrentMediaPlaybackItemChangedEventArgs_type{py::register_python_type(module.get(), &type_spec_CurrentMediaPlaybackItemChangedEventArgs, object_bases.get(), nullptr)};
    if (!CurrentMediaPlaybackItemChangedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaBreak_type{py::register_python_type(module.get(), &type_spec_MediaBreak, object_bases.get(), nullptr)};
    if (!MediaBreak_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaBreakEndedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaBreakEndedEventArgs, object_bases.get(), nullptr)};
    if (!MediaBreakEndedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaBreakManager_type{py::register_python_type(module.get(), &type_spec_MediaBreakManager, object_bases.get(), nullptr)};
    if (!MediaBreakManager_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaBreakSchedule_type{py::register_python_type(module.get(), &type_spec_MediaBreakSchedule, object_bases.get(), nullptr)};
    if (!MediaBreakSchedule_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaBreakSeekedOverEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaBreakSeekedOverEventArgs, object_bases.get(), nullptr)};
    if (!MediaBreakSeekedOverEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaBreakSkippedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaBreakSkippedEventArgs, object_bases.get(), nullptr)};
    if (!MediaBreakSkippedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaBreakStartedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaBreakStartedEventArgs, object_bases.get(), nullptr)};
    if (!MediaBreakStartedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaItemDisplayProperties_type{py::register_python_type(module.get(), &type_spec_MediaItemDisplayProperties, object_bases.get(), nullptr)};
    if (!MediaItemDisplayProperties_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackAudioTrackList_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackAudioTrackList, object_bases.get(), nullptr)};
    if (!MediaPlaybackAudioTrackList_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackCommandManager_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackCommandManager, object_bases.get(), nullptr)};
    if (!MediaPlaybackCommandManager_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackCommandManagerAutoRepeatModeReceivedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackCommandManagerAutoRepeatModeReceivedEventArgs, object_bases.get(), nullptr)};
    if (!MediaPlaybackCommandManagerAutoRepeatModeReceivedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackCommandManagerCommandBehavior_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackCommandManagerCommandBehavior, object_bases.get(), nullptr)};
    if (!MediaPlaybackCommandManagerCommandBehavior_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackCommandManagerFastForwardReceivedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackCommandManagerFastForwardReceivedEventArgs, object_bases.get(), nullptr)};
    if (!MediaPlaybackCommandManagerFastForwardReceivedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackCommandManagerNextReceivedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackCommandManagerNextReceivedEventArgs, object_bases.get(), nullptr)};
    if (!MediaPlaybackCommandManagerNextReceivedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackCommandManagerPauseReceivedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackCommandManagerPauseReceivedEventArgs, object_bases.get(), nullptr)};
    if (!MediaPlaybackCommandManagerPauseReceivedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackCommandManagerPlayReceivedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackCommandManagerPlayReceivedEventArgs, object_bases.get(), nullptr)};
    if (!MediaPlaybackCommandManagerPlayReceivedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackCommandManagerPositionReceivedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackCommandManagerPositionReceivedEventArgs, object_bases.get(), nullptr)};
    if (!MediaPlaybackCommandManagerPositionReceivedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackCommandManagerPreviousReceivedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackCommandManagerPreviousReceivedEventArgs, object_bases.get(), nullptr)};
    if (!MediaPlaybackCommandManagerPreviousReceivedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackCommandManagerRateReceivedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackCommandManagerRateReceivedEventArgs, object_bases.get(), nullptr)};
    if (!MediaPlaybackCommandManagerRateReceivedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackCommandManagerRewindReceivedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackCommandManagerRewindReceivedEventArgs, object_bases.get(), nullptr)};
    if (!MediaPlaybackCommandManagerRewindReceivedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackCommandManagerShuffleReceivedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackCommandManagerShuffleReceivedEventArgs, object_bases.get(), nullptr)};
    ...
    py::pyobj_handle MediaPlaybackItem_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackItem, object_bases.get(), reinterpret_cast<PyTypeObject*>(type_MediaPlaybackItem_Static.get()))};
    if (!MediaPlaybackItem_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackItemError_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackItemError, object_bases.get(), nullptr)};
    if (!MediaPlaybackItemError_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackItemFailedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackItemFailedEventArgs, object_bases.get(), nullptr)};
    if (!MediaPlaybackItemFailedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackItemOpenedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackItemOpenedEventArgs, object_bases.get(), nullptr)};
    if (!MediaPlaybackItemOpenedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackList_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackList, object_bases.get(), nullptr)};
    if (!MediaPlaybackList_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackSession_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackSession, object_bases.get(), nullptr)};
    if (!MediaPlaybackSession_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackSessionBufferingStartedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackSessionBufferingStartedEventArgs, object_bases.get(), nullptr)};
    if (!MediaPlaybackSessionBufferingStartedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackSessionOutputDegradationPolicyState_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackSessionOutputDegradationPolicyState, object_bases.get(), nullptr)};
    if (!MediaPlaybackSessionOutputDegradationPolicyState_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackSphericalVideoProjection_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackSphericalVideoProjection, object_bases.get(), nullptr)};
    if (!MediaPlaybackSphericalVideoProjection_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackTimedMetadataTrackList_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackTimedMetadataTrackList, object_bases.get(), nullptr)};
    if (!MediaPlaybackTimedMetadataTrackList_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlaybackVideoTrackList_type{py::register_python_type(module.get(), &type_spec_MediaPlaybackVideoTrackList, object_bases.get(), nullptr)};
    if (!MediaPlaybackVideoTrackList_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlayer_type{py::register_python_type(module.get(), &type_spec_MediaPlayer, object_bases.get(), nullptr)};
    if (!MediaPlayer_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlayerDataReceivedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlayerDataReceivedEventArgs, object_bases.get(), nullptr)};
    if (!MediaPlayerDataReceivedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlayerFailedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlayerFailedEventArgs, object_bases.get(), nullptr)};
    if (!MediaPlayerFailedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlayerRateChangedEventArgs_type{py::register_python_type(module.get(), &type_spec_MediaPlayerRateChangedEventArgs, object_bases.get(), nullptr)};
    if (!MediaPlayerRateChangedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle MediaPlayerSurface_type{py::register_python_type(module.get(), &type_spec_MediaPlayerSurface, object_bases.get(), nullptr)};
    if (!MediaPlayerSurface_type)
    {
        return nullptr;
    }

    py::pyobj_handle PlaybackMediaMarker_type{py::register_python_type(module.get(), &type_spec_PlaybackMediaMarker, object_bases.get(), nullptr)};
    if (!PlaybackMediaMarker_type)
    {
        return nullptr;
    }

    py::pyobj_handle PlaybackMediaMarkerReachedEventArgs_type{py::register_python_type(module.get(), &type_spec_PlaybackMediaMarkerReachedEventArgs, object_bases.get(), nullptr)};
    if (!PlaybackMediaMarkerReachedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle PlaybackMediaMarkerSequence_type{py::register_python_type(module.get(), &type_spec_PlaybackMediaMarkerSequence, object_bases.get(), nullptr)};
    if (!PlaybackMediaMarkerSequence_type)
    {
        return nullptr;
    }

    py::pyobj_handle TimedMetadataPresentationModeChangedEventArgs_type{py::register_python_type(module.get(), &type_spec_TimedMetadataPresentationModeChangedEventArgs, object_bases.get(), nullptr)};
    if (!TimedMetadataPresentationModeChangedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle IMediaEnginePlaybackSource_type{py::register_python_type(module.get(), &type_spec_IMediaEnginePlaybackSource, object_bases.get(), nullptr)};
    if (!IMediaEnginePlaybackSource_type)
    {
        return nullptr;
    }

    py::pyobj_handle IMediaPlaybackSource_type{py::register_python_type(module.get(), &type_spec_IMediaPlaybackSource, object_bases.get(), nullptr)};
