...
PyMODINIT_FUNC PyInit__winrt_Windows_Media_Transcoding(void) noexcept
{
    ...
    state->type_MediaTranscoder = py::register_python_type(module.get(), type_name_MediaTranscoder, &type_spec_MediaTranscoder, object_bases.get(), nullptr);
    if (!state->type_MediaTranscoder)
    {
        return nullptr;
    }

    state->type_PrepareTranscodeResult = py::register_python_type(module.get(), type_name_PrepareTranscodeResult, &type_spec_PrepareTranscodeResult, object_bases.get(), nullptr);
