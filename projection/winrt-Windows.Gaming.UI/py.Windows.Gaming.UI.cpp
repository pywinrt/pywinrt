...
PyMODINIT_FUNC PyInit__winrt_windows_gaming_ui(void) noexcept
{
    ...
    py::pyobj_handle GameBar_type{py::register_python_type(module.get(), &type_spec_GameBar, object_bases.get(), reinterpret_cast<PyTypeObject*>(type_GameBar_Static.get()))};
    if (!GameBar_type)
    {
        return nullptr;
    }

    py::pyobj_handle GameChatMessageReceivedEventArgs_type{py::register_python_type(module.get(), &type_spec_GameChatMessageReceivedEventArgs, object_bases.get(), nullptr)};
    ...
    py::pyobj_handle GameChatOverlay_type{py::register_python_type(module.get(), &type_spec_GameChatOverlay, object_bases.get(), reinterpret_cast<PyTypeObject*>(type_GameChatOverlay_Static.get()))};
    if (!GameChatOverlay_type)
    {
        return nullptr;
    }

    py::pyobj_handle GameChatOverlayMessageSource_type{py::register_python_type(module.get(), &type_spec_GameChatOverlayMessageSource, object_bases.get(), nullptr)};
    if (!GameChatOverlayMessageSource_type)
    {
        return nullptr;
    }

    py::pyobj_handle GameUIProviderActivatedEventArgs_type{py::register_python_type(module.get(), &type_spec_GameUIProviderActivatedEventArgs, object_bases.get(), nullptr)};
