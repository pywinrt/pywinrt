...
PyMODINIT_FUNC PyInit__winrt_windows_media_import_(void) noexcept
{
    ...
    py::pyobj_handle PhotoImportDeleteImportedItemsFromSourceResult_type{py::register_python_type(module.get(), &type_spec_PhotoImportDeleteImportedItemsFromSourceResult, object_bases.get(), nullptr)};
    if (!PhotoImportDeleteImportedItemsFromSourceResult_type)
    {
        return nullptr;
    }

    py::pyobj_handle PhotoImportFindItemsResult_type{py::register_python_type(module.get(), &type_spec_PhotoImportFindItemsResult, object_bases.get(), nullptr)};
    if (!PhotoImportFindItemsResult_type)
    {
        return nullptr;
    }

    py::pyobj_handle PhotoImportImportItemsResult_type{py::register_python_type(module.get(), &type_spec_PhotoImportImportItemsResult, object_bases.get(), nullptr)};
    if (!PhotoImportImportItemsResult_type)
    {
        return nullptr;
    }

    py::pyobj_handle PhotoImportItem_type{py::register_python_type(module.get(), &type_spec_PhotoImportItem, object_bases.get(), nullptr)};
    if (!PhotoImportItem_type)
    {
        return nullptr;
    }

    py::pyobj_handle PhotoImportItemImportedEventArgs_type{py::register_python_type(module.get(), &type_spec_PhotoImportItemImportedEventArgs, object_bases.get(), nullptr)};
    ...
    py::pyobj_handle PhotoImportManager_type{py::register_python_type(module.get(), &type_spec_PhotoImportManager, object_bases.get(), reinterpret_cast<PyTypeObject*>(type_PhotoImportManager_Static.get()))};
    if (!PhotoImportManager_type)
    {
        return nullptr;
    }

    py::pyobj_handle PhotoImportOperation_type{py::register_python_type(module.get(), &type_spec_PhotoImportOperation, object_bases.get(), nullptr)};
    if (!PhotoImportOperation_type)
    {
        return nullptr;
    }

    py::pyobj_handle PhotoImportSelectionChangedEventArgs_type{py::register_python_type(module.get(), &type_spec_PhotoImportSelectionChangedEventArgs, object_bases.get(), nullptr)};
    if (!PhotoImportSelectionChangedEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle PhotoImportSession_type{py::register_python_type(module.get(), &type_spec_PhotoImportSession, object_bases.get(), nullptr)};
    if (!PhotoImportSession_type)
    {
        return nullptr;
    }

    py::pyobj_handle PhotoImportSidecar_type{py::register_python_type(module.get(), &type_spec_PhotoImportSidecar, object_bases.get(), nullptr)};
    ...
    py::pyobj_handle PhotoImportSource_type{py::register_python_type(module.get(), &type_spec_PhotoImportSource, object_bases.get(), reinterpret_cast<PyTypeObject*>(type_PhotoImportSource_Static.get()))};
    if (!PhotoImportSource_type)
    {
        return nullptr;
    }

    py::pyobj_handle PhotoImportStorageMedium_type{py::register_python_type(module.get(), &type_spec_PhotoImportStorageMedium, object_bases.get(), nullptr)};
    if (!PhotoImportStorageMedium_type)
    {
        return nullptr;
    }

    py::pyobj_handle PhotoImportVideoSegment_type{py::register_python_type(module.get(), &type_spec_PhotoImportVideoSegment, object_bases.get(), nullptr)};
    if (!PhotoImportVideoSegment_type)
    {
        return nullptr;
    }

    py::pyobj_handle PhotoImportProgress_type{py::register_python_type(module.get(), &type_spec_PhotoImportProgress, nullptr, nullptr)};
