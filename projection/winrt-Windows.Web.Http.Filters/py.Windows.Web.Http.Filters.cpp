...
PyMODINIT_FUNC PyInit__winrt_Windows_Web_Http_Filters(void) noexcept
{
    ...
    state->type_HttpBaseProtocolFilter = py::register_python_type(module.get(), type_name_HttpBaseProtocolFilter, &type_spec_HttpBaseProtocolFilter, object_bases.get(), nullptr);
    if (!state->type_HttpBaseProtocolFilter)
    {
        return nullptr;
    }

    state->type_HttpCacheControl = py::register_python_type(module.get(), type_name_HttpCacheControl, &type_spec_HttpCacheControl, object_bases.get(), nullptr);
    if (!state->type_HttpCacheControl)
    {
        return nullptr;
    }

    state->type_HttpServerCustomValidationRequestedEventArgs = py::register_python_type(module.get(), type_name_HttpServerCustomValidationRequestedEventArgs, &type_spec_HttpServerCustomValidationRequestedEventArgs, object_bases.get(), nullptr);
    if (!state->type_HttpServerCustomValidationRequestedEventArgs)
    {
        return nullptr;
    }

    state->type_IHttpFilter = py::register_python_type(module.get(), type_name_IHttpFilter, &type_spec_IHttpFilter, object_bases.get(), nullptr);
