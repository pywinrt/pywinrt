...
PyMODINIT_FUNC PyInit__winrt_Windows_Storage_BulkAccess(void) noexcept
{
    ...
    state->type_FileInformation = py::register_python_type(module.get(), type_name_FileInformation, &type_spec_FileInformation, object_bases.get(), nullptr);
    if (!state->type_FileInformation)
    {
        return nullptr;
    }

    state->type_FileInformationFactory = py::register_python_type(module.get(), type_name_FileInformationFactory, &type_spec_FileInformationFactory, object_bases.get(), nullptr);
    if (!state->type_FileInformationFactory)
    {
        return nullptr;
    }

    state->type_FolderInformation = py::register_python_type(module.get(), type_name_FolderInformation, &type_spec_FolderInformation, object_bases.get(), nullptr);
    if (!state->type_FolderInformation)
    {
        return nullptr;
    }

    state->type_IStorageItemInformation = py::register_python_type(module.get(), type_name_IStorageItemInformation, &type_spec_IStorageItemInformation, object_bases.get(), nullptr);
