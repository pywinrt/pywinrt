...
PyMODINIT_FUNC PyInit__winrt_windows_storage_bulkaccess(void) noexcept
{
    ...
    py::pyobj_handle FileInformation_type{py::register_python_type(module.get(), &type_spec_FileInformation, object_bases.get(), nullptr)};
    if (!FileInformation_type)
    {
        return nullptr;
    }

    py::pyobj_handle FileInformationFactory_type{py::register_python_type(module.get(), &type_spec_FileInformationFactory, object_bases.get(), nullptr)};
    if (!FileInformationFactory_type)
    {
        return nullptr;
    }

    py::pyobj_handle FolderInformation_type{py::register_python_type(module.get(), &type_spec_FolderInformation, object_bases.get(), nullptr)};
    if (!FolderInformation_type)
    {
        return nullptr;
    }

    py::pyobj_handle IStorageItemInformation_type{py::register_python_type(module.get(), &type_spec_IStorageItemInformation, object_bases.get(), nullptr)};
