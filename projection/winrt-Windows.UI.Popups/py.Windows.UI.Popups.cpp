...
PyMODINIT_FUNC PyInit__winrt_Windows_UI_Popups(void) noexcept
{
    ...
    state->type_MessageDialog = py::register_python_type(module.get(), type_name_MessageDialog, &type_spec_MessageDialog, object_bases.get(), nullptr);
    if (!state->type_MessageDialog)
    {
        return nullptr;
    }

    state->type_PopupMenu = py::register_python_type(module.get(), type_name_PopupMenu, &type_spec_PopupMenu, object_bases.get(), nullptr);
    if (!state->type_PopupMenu)
    {
        return nullptr;
    }

    state->type_UICommand = py::register_python_type(module.get(), type_name_UICommand, &type_spec_UICommand, object_bases.get(), nullptr);
    if (!state->type_UICommand)
    {
        return nullptr;
    }

    state->type_UICommandSeparator = py::register_python_type(module.get(), type_name_UICommandSeparator, &type_spec_UICommandSeparator, object_bases.get(), nullptr);
    if (!state->type_UICommandSeparator)
    {
        return nullptr;
    }

    state->type_IUICommand = py::register_python_type(module.get(), type_name_IUICommand, &type_spec_IUICommand, object_bases.get(), nullptr);
