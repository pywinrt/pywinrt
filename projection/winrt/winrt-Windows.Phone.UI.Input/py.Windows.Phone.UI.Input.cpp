...
PyMODINIT_FUNC PyInit__winrt_windows_phone_ui_input(void) noexcept
{
    ...
    py::pytype_handle BackPressedEventArgs_type{py::register_python_type(module.get(), &type_spec_BackPressedEventArgs, object_bases.get(), inspectable_meta_type)};
    if (!BackPressedEventArgs_type)
    {
        return nullptr;
    }

    py::pytype_handle CameraEventArgs_type{py::register_python_type(module.get(), &type_spec_CameraEventArgs, object_bases.get(), inspectable_meta_type)};
    if (!CameraEventArgs_type)
    {
        return nullptr;
    }

    py::pyobj_handle HardwareButtons_Static_bases{PyTuple_Pack(1, reinterpret_cast<PyObject*>(inspectable_meta_type))};
    if (!HardwareButtons_Static_bases)
    {
        return nullptr;
    }

    py::pyobj_handle type_HardwareButtons_Static{PyType_FromSpecWithBases(&type_spec_HardwareButtons_Static, HardwareButtons_Static_bases.get())};
