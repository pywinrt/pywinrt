...
PyMODINIT_FUNC PyInit__winrt_Windows_Security_ExchangeActiveSyncProvisioning(void) noexcept
{
    ...
    state->type_EasClientDeviceInformation = py::register_python_type(module.get(), type_name_EasClientDeviceInformation, &type_spec_EasClientDeviceInformation, object_bases.get(), nullptr);
    if (!state->type_EasClientDeviceInformation)
    {
        return nullptr;
    }

    state->type_EasClientSecurityPolicy = py::register_python_type(module.get(), type_name_EasClientSecurityPolicy, &type_spec_EasClientSecurityPolicy, object_bases.get(), nullptr);
    if (!state->type_EasClientSecurityPolicy)
    {
        return nullptr;
    }

    state->type_EasComplianceResults = py::register_python_type(module.get(), type_name_EasComplianceResults, &type_spec_EasComplianceResults, object_bases.get(), nullptr);
