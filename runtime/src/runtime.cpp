...
{
    const auto kind = info.kind == member_kind::method     ? "method"
                      : info.kind == member_kind::property ? "property"
                                                           : "event";
    ...
    // which is the more useful thing to say when it does not, so it is asked
    // here, on the failure path, where its cost does not matter. It has no
    // answer at all if the metadata cannot be resolved, hence the optional.
    auto in_metadata = std::optional<bool>{};
    ...
    if (info.type_name)
    {
        try
        {
            ...
            const auto type = winrt::to_hstring(info.type_name);

            // Metadata is resolved through the package graph and the app
            // directory as well as the Windows metadata directory, so a type
            // outside the Windows namespace is only found when the framework
            // package that declares it is registered for this process. If
            // the type itself is not found, the member being missing from
            // the metadata says nothing about which versions have it.
            if (ApiInformation::IsTypePresent(type))
            {
                const auto member = winrt::to_hstring(info.member_name);

                switch (info.kind)
                {
                case member_kind::method:
                    in_metadata
                        = ApiInformation::IsMethodPresent(type, member, info.arg_count);
                    break;
                case member_kind::property:
                    in_metadata = ApiInformation::IsPropertyPresent(type, member);
                    break;
                case member_kind::event:
                    in_metadata = ApiInformation::IsEventPresent(type, member);
                    break;
                }
            }
        }
